...
const contractInteractions = () => {
    ...
    // balanceOfTokens keeps track of number of tokens owned by an address
    const [balanceOfTokens, setBalanceOfTokens] = useState(
    zero
    );
    ...
    const supportArtist = async (artistAddress ,amount) => {
        try {
        // We need a Signer here since this is a 'write' transaction.
        // Create an instance of tokenContract
        const signer = await getProviderOrSigner(true);
        ...
        const tokenContract = new Contract(
            ...
            const tx = await tokenContract.supportArtist(artistAddress, {
                // value signifies the cost of one crypto dev token which is "0.001" eth.
                // We are parsing `0.001` string to ether using the utils library from ethers.js
            ...
            await tx.wait();
            setLoading(false);
            window.alert("Sucessfully sent eth to artist");
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const registerImage = async (IPFSLink) => {
        ...
        const tx = await tokenContract.registerImage(IPFSLink);
            ...
            await tx.wait();
            setLoading(false);
            window.alert("Sucessfully added the image on the blockchain");
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const getLinks = async () => {
        try {
            ...
            const tokenContract = new Contract(
                ...
                //return type?    
                const tx = await tokenContract.getLinks();
                setLoading(true);
                await tx.wait();
                setLoading(false);
                await getOwner();
